...
<body class="gradient-bg dark:dark-gradient-bg min-h-screen">
    <!-- Navigation -->
    <nav class="glass dark:dark-glass sticky top-0 z-50">
                ...
                <div class="flex items-center">
                    ...
                    <div class="hidden md:block ml-10">
                        <div class="flex items-baseline space-x-4">
                            ...
                            <span class="text-gray-500 dark:text-gray-400 text-sm">{{ vps_ip }}</span>
                        </div>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <button onclick="toggleTheme()" class="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300">
                    ...
                    <form method="POST" action="/apply" onsubmit="return confirm('Apply configuration changes? This will reload the service.');">
                        <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg font-medium transition duration-200 transform hover:-translate-y-0.5">
                            <div class="flex items-center">
                    ...
                    <a href="/logout" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition duration-200">
                        Logout
                    </a>
                </div>
            </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="glass dark:dark-glass rounded-2xl p-6 shadow-soft">
                <div class="flex items-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <form method="POST" action="/save" id="userForm" class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="username" required
                                   class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                   placeholder="Enter username">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="password" required
                                   class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                   placeholder="Enter password">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Automatically copied when created</p>
                        </div>
            ...
            <div class="lg:col-span-2">
                <div class="glass dark:dark-glass rounded-2xl shadow-soft overflow-hidden">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center space-x-2">
                                            <code class="px-3 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 font-mono text-sm">{{ r.password }}</code>
                                            <button onclick="copyToClipboard('{{ r.password }}')" 
                                                    class="px-3 py-1 text-xs bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition duration-200">
                                                Copy
                                            </button>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <button onclick="editUser('{{ r.username }}', '{{ r.password }}', '{{ r.expires }}')"
                                                    class="px-3 py-1 bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:hover:bg-blue-800 text-blue-700 dark:text-blue-300 rounded-lg transition duration-200">
                                                Edit
                                            </button>
                                            <form method="POST" action="/del/{{ r.id }}" onsubmit="return confirmDelete('{{ r.username }}')" class="inline">
                                                <button type="submit"
                                                        class="px-3 py-1 bg-red-100 hover:bg-red-200 dark:bg-red-900 dark:hover:bg-red-800 text-red-700 dark:text-red-300 rounded-lg transition duration-200">
                                                    Delete
                                                </button>
                                            </form>
                                        </div>
    ...
    {% with messages = get_flashed_messages(with_categories=true) %}
        {% if messages %}
            {% for category, message in messages %}
                <div id="flash-{{ loop.index }}" class="fixed bottom-4 right-4 z-50">
                    <div class="glass dark:dark-glass rounded-xl p-4 shadow-soft max-w-md">
                        <div class="flex items-center">
    ...
    <script>
        ...
        server_ts=server_ts, uptime=uptime_output)
...
if command -v sudo >/dev/null 2>&1; then
  echo "🚀 Starting installation with Modern UI..."
  sudo ./zi.sh
else
  echo "🚀 Starting installation with Modern UI..."
  ./zi.sh
